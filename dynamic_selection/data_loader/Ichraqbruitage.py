...
###############################################
    ...
    def predict_class(self, datapoint):
        ...
        with torch.no_grad():
            inputs = torch.from_numpy(datapoint).unsqueeze(0).permute(0, 3, 1, 2).float()
            outputs = self.prediction_model(inputs)
            _, predicted = torch.max(outputs.data, 1)
            confidence = torch.softmax(outputs, dim=1)[0, predicted] * 100
        print("rejoice! For a prediction has occured ",predicted.item(), confidence.item() )
    ...
    def symmetric_noise(self):
      ...
      compteur_chat = 0
      compteur_chien = 0
      compteur_max = self.cfg_trainer['percent'] * len(sorted_indices)*0.1
